    ...
    >
      ...
      >
        ...
        <motion.img
          ...
          loading="lazy"
          decoding="async"
          {...generateImageSizeProps({ image: project.src })}
          className={cn(
            'pointer-events-none absolute inset-0 -z-10 h-full w-full object-contain transition-[transform,opacity,filter] duration-700',
            isDisabled
...
function ProjectCarousel({ projects }: { projects: any }) {
  ...
  const isLastSlide = currentSlide === (projects?.length || 0) - 1;
      ...
      <div className="relative py-8">
        ...
        >
          ...
          <button
            ...
            aria-controls="project-carousel"
            aria-disabled={isLastSlide} // Improved accessibility
            disabled={isLastSlide}
            className={`pointer-events-auto aspect-square h-fit rounded-full border border-neutrals-600 bg-neutrals-900/90 p-4 text-neutrals-100 drop-shadow-md backdrop-blur-sm ${isLastSlide ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`} // Conditional classes
          >
